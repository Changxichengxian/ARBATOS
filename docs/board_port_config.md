...
- 运行参数和轴电机装配还是继续放 `config.c` / `config.h`。
...
- PID
- 控制周期
- 每个轴装什么电机、CAN ID 是多少，也就是 `g_config.motor`
- 其他运行时参数
...
- `bsp_board_port_usage.h` 管“哪个串口/CAN/RS485 口给谁用”。
- `config.c` 里的 `g_config.motor` 管“底盘、云台、射击这些轴装什么电机”。
