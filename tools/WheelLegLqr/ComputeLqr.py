...
def torque_diagnostics(
    ...
    lines = [
        "### 短腿静态力矩检查",
        "",
        f"以下检查使用最短拟合腿长 L0={length:.3f} m。",
        "它们只是起始瞬间的线性估算，不能保证实车一定能恢复。",
        "",
        "| 静态误差 | 轮端力矩 T | 髋部虚拟力矩 Tp |",
        "| --- | ---: | ---: |",
        f"| 车体俯仰 phi = 10 deg | {wheel[4] * deg10:.4f} N*m | {hip[4] * deg10:.4f} N*m |",
        f"| 腿角 theta = 10 deg | {wheel[0] * deg10:.4f} N*m | {hip[0] * deg10:.4f} N*m |",
        f"| 轮子位置 x = 10 cm | {wheel[2] * 0.10:.4f} N*m | {hip[2] * 0.10:.4f} N*m |",
        f"| 轮速 dx = 0.5 m/s | {wheel[3] * 0.50:.4f} N*m | {hip[3] * 0.50:.4f} N*m |",
        "",
        "| 轮端力矩限幅检查 | 仅车体俯仰 | 仅腿角 |",
        "| --- | ---: | ---: |",
        f"| 连续 {params.wheel_torque_continuous:.2f} N*m | {limit_angle(params.wheel_torque_continuous, wheel[4]):.1f} deg | {limit_angle(params.wheel_torque_continuous, wheel[0]):.1f} deg |",
        f"| 峰值 {params.wheel_torque_peak:.2f} N*m | {limit_angle(params.wheel_torque_peak, wheel[4]):.1f} deg | {limit_angle(params.wheel_torque_peak, wheel[0]):.1f} deg |",
        "",
    ]
    return lines
...
def report_case(case_name: str, params: WheelLegParams, Q: np.ndarray, R: np.ndarray) -> str:
    ...
    lines = [
        ...
        f"- 连续水平加速度估算 = {continuous_accel:.3f} m/s^2",
        f"- 峰值水平加速度估算 = {peak_accel:.3f} m/s^2",
        "",
        "### 权重矩阵 Q/R",
...
def main() -> None:
    ...
    output = [
        "# 3510 小轮腿参数计算报告（LQR）",
        "",
        "本文件由 `tools/WheelLegLqr/ComputeLqr.py` 生成。",
        "",
        "## VMC 几何参数",
        "```c",
    ...
    out_path = Path(__file__).with_name("轮腿参数计算报告.md")
