...
- `MDK-ARM/`：Keil 工程
- `target/HERO/`：HERO 目标配置和健康监测，包含 `config.c`、`config.h`、`detect_task.c`
...
- 命令格式：`<id>:<value>\r\n`
- `id` 对应 `target/HERO/config.c` 中每行末尾的 `[ID]` 注释
...
- 遥测信号枚举：`aux_telem_sig_e`（见 `target/HERO/config.h`）
...
相关参数：`target/HERO/config.c` 的 `g_config.power.*`
