...
/*
 * UART1 临时改参：发送 "<id>:<value>"（例如 "1:1000"）
 * - 编号见本文件 g_app_config 初始化处每行末尾的 [ID] 注释
 * - 未标 [ID] 的参数：只在 init 使用 / 未运行时应用（UART1 不支持改）
 * - 仅修改 RAM 中的 g_app_config，重启后恢复默认值
 */

// 默认配置：如需调整参数请修改此处，或在启动时由外部存储覆盖。
// 注意：本表用于编译期默认值，运行时可按需复制到 RAM 并覆写。
    ...
    .gimbal =
        {
            ...
            // - 默认以 P 为主（接近 YAW 的量级），I 默认先关；若需要重力补偿再通过 UART1 逐步加一点 Ki
    ...
    .shoot =
        {
            ...
            .fric_motor_dir = {0, 0, 0, 0},      // [122]0x201 [123]0x202 [124]0x203 [125]0x204 | CARRIER 默认关闭摩擦轮
    ...
    .detect =
        {
            ...
            .enable_mask = 0x1F1F,    // [209] 关闭 yaw/pitch/trigger 离线检测，保留底盘和板载部分
    ...
    .input =
        {
            ...
            .axis =
                {
                    [APP_AXIS_CHASSIS_X] = {.rc_ch = 3u, .invert = 0u},     // [320] ch [321] invert
                    [APP_AXIS_CHASSIS_Y] = {.rc_ch = 2u, .invert = 0u},     // [322] ch [323] invert
                    [APP_AXIS_CHASSIS_WZ] = {.rc_ch = 0u, .invert = 0u},    // [324] ch [325] invert
                    [APP_AXIS_GIMBAL_YAW] = {.rc_ch = 0u, .invert = 0u},    // [326] ch [327] invert
                    [APP_AXIS_GIMBAL_PITCH] = {.rc_ch = 1u, .invert = 0u},  // [328] ch [329] invert
                    [APP_AXIS_CALIB_0] = {.rc_ch = 0u, .invert = 0u},       // [330] ch [331] invert
                    [APP_AXIS_CALIB_1] = {.rc_ch = 1u, .invert = 0u},       // [332] ch [333] invert
                    [APP_AXIS_CALIB_2] = {.rc_ch = 2u, .invert = 0u},       // [334] ch [335] invert
                    [APP_AXIS_CALIB_3] = {.rc_ch = 3u, .invert = 0u},       // [336] ch [337] invert
                },
            .sw =
                {
                    [APP_SW_GIMBAL_MODE] = {.rc_sw = 0u, .invert = 0u},     // [338] sw [339] invert
                    [APP_SW_CHASSIS_MODE] = {.rc_sw = 0u, .invert = 0u},    // [340] sw [341] invert
                    [APP_SW_SHOOT_MODE] = {.rc_sw = 1u, .invert = 0u},      // [342] sw [343] invert
                    [APP_SW_CALIB_L] = {.rc_sw = 0u, .invert = 0u},         // [344] sw [345] invert
                    [APP_SW_CALIB_R] = {.rc_sw = 1u, .invert = 0u},         // [346] sw [347] invert
                },
        },

    .test =
        {
            .mode = TEST_MODE_CHASSIS_ONLY, // [244] A 板 carrier 默认只开底盘
            // 可直接复制的取值（枚举值 / UART1 value）：
            // 0: TEST_MODE_NONE,
	    ...
	    // UART1 VOFA+/FireWater (JustFloat) 遥测：N*fp32 + INF 尾（实时/带宽有限）
	    ...
	    // - 自定义列表：channel_num!=0 时，发送 channel_map[0..channel_num-1]（元素为“信号编号(ID)”=uart1_telem_sig_e 数值）
