...
`target/` 放“这台机器人是谁”的内容。这里描述目标本身，不描述 Keil 工程怎么编译，也不描述某块开发板有哪些引脚。
...
| `MC02_BASE` | H7 接板和机械臂实验 | `config.c`、`config.h`、`detect_task.c`、`arm_motor_table.c` |
...
判断标准很简单：如果换一台同板子的机器人也要改它，它大概率属于 `target/`；如果换一块板子才要改它，它大概率属于 `boards/`。
