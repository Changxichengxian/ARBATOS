...
config_t g_config = {
            ...
            .yaw =
                {
                    ...
                    .control_mode = MOTOR_CONTROL_MODE_MIT,
    ...
    .shoot =
        {
            ...
            .fric_motor_dir = {-1, 1, 0, 0},    // 两路摩擦轮方向：0x201 / 0x202，后两路关闭
    ...
    .detect =
        {
            ...
            .enable_mask = 0x1FFF,    // 在线检测使能位：bits[0..12]=1，OLED(bit13)=0
    ...
    .test =
        {
            .mode = TEST_MODE_ENTERTAIN, // [244] 测试模式默认值，可通过 AUX 口临时改
