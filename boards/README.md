...
| Board | 芯片 | 说明 |
|---|---|---|
| `DjiAF427` | STM32F427II | DJI A 板，含原理图核对记录 |
| `DjiCF407` | STM32F407 | DJI C 板，保留供新车型使用 |
| `DmMc02H7` | STM32H723 | 达妙 MC02 H7 开发板适配 |

每块板的 `zephyr/` 子目录放设备树、引脚和调试器配置；`boards/dts/` 放共用设备树属性定义。删除车型时不删除这里的开发板支持。
...
- 正式构建的目标配置、显式源码清单和启动入口：放 `projects/`。
- 跨板复用的控制逻辑和协议驱动：放 `shared/`。
- 跨板复用的外设封装：当前 Zephyr 适配放 `shared/zephyr/`，历史 HAL 接口保留在 `shared/hal/`；这里的 `bsp/` 只放具体板子的端口和配置。
