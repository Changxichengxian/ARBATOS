...
static void BspRcRecordRx(uint16_t size);
...
void USART3_IRQHandler(void)
{
    ...
    if (rc_diag.uart_sr & UART_FLAG_IDLE)
    {
        ...
        if ((BSP_RC_DMA_HANDLE.Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            BspRcRecordRx(this_time_rx_len);
            BspRcSbusOnFrameIsr(sbus_rx_dma_buf[0], this_time_rx_len);
        }
        else
        {
            __HAL_DMA_DISABLE(&BSP_RC_DMA_HANDLE);
            ...
            BspRcRecordRx(this_time_rx_len);
            BspRcSbusOnFrameIsr(sbus_rx_dma_buf[1], this_time_rx_len);
        }
    }
}
