...
static TaskHandle_t s_ins_task_handle;
...
static uint8_t GyroZeroApplyDelta = 0u;
...
static void GyroZeroApplyOffsetDelta(const fp32 offset_delta[3])
{
...
static void GyroZeroUpdateAccelRef(const GyroZeroCaliSampleState *state)
{
    gyro_boot_accel_norm_ref = GyroZeroCaliAccelNormAvg(state, GRAVITY_EARTH);
    gyro_boot_accel_norm_valid = 1;
}

static void GyroZeroSampleDoneCb(const GyroZeroCaliSampleState *state, void *ctx)
{
    (void)ctx;
    GyroZeroUpdateAccelRef(state);
}

static void GyroZeroApplyOffsetCb(const fp32 offset[3], void *ctx)
{
    (void)ctx;
    if (GyroZeroApplyDelta != 0u)
    {
        GyroZeroApplyOffsetDelta(offset);
    }
    else
    {
        GyroZeroApplyOffset(offset);
...
static void GyroZeroRuntimeUpdate(const fp32 gyro_raw[3], const fp32 accel_raw[3], fp32 temp_c, uint32_t now_ms)
{
    ...
    GyroZeroApplyDelta = (test_mode_active == 0u) ? 1u : 0u;
    ...
    const GyroZeroCaliRuntimeCfg cfg = {
        ...
        .sample_done = GyroZeroSampleDoneCb,
        .ctx = NULL,
    };
    GyroZeroCaliRuntimeUpdate(&GyroZeroState, &cfg, gyro_sample, accel_raw);
    GyroZeroApplyDelta = 0u;
}
...
void InsTask(void const *pvParameters)
{
    ...
    imu_fusion_reset((imu_fusion_mode_e)imu_cfg->fusion_mode, accel_fliter_3, INS_mag);
    //get the handle of task
    //获取当前任务的任务句柄，
    s_ins_task_handle = xTaskGetHandle(pcTaskGetName(NULL));
    ...
    while (1)
    {
        ...
        GyroZeroRuntimeUpdate(bmi088_real_data.gyro, bmi088_real_data.accel, bmi088_real_data.temp, now_ms);
...
void DMA2_Stream2_IRQHandler(void)
{
    ...
    if(__HAL_DMA_GET_FLAG(hspi1.hdmarx, __HAL_DMA_GET_TC_FLAG_INDEX(hspi1.hdmarx)) != RESET)
    {
        ...
        if(gyro_update_flag & (1 << IMU_UPDATE_SHFITS))
        {
            ...
            // Wake InsTask directly from DMA ISR (avoid occupying EXTI0/GPIO0).
            if (s_ins_task_handle != NULL && xTaskGetSchedulerState() != taskSCHEDULER_NOT_STARTED)
            {
