...
#include "Watch.h"
#define IMU_WATCH_ERROR() WatchTaskError(WATCH_TASK_IMU)
#define IMU_WATCH_WAIT() WatchTaskWait(WATCH_TASK_IMU)
#define IMU_WATCH_BEAT() WatchTaskBeat(WATCH_TASK_IMU)
#define IMU_WATCH_TIMEOUT() WatchTaskTimeout(WATCH_TASK_IMU)
#else
...
void InsTask(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (got_raw)
        {
            ...
            GyroZeroRuntimeUpdate(gyro_raw, accel_raw, temp_c, now_ms);
            ...
            InsSnapshotPublishCurrent(now_ms, temp_c);

            sdlog_imu_base_sample_t pkt = {0};
            SdLogImuBaseSampleSet(&pkt, INS_quat, INS_gyro, INS_accel, temp_c);
