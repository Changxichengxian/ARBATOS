...
- [任务与数据流](#任务与数据流)
- [外设与通信分配](#外设与通信分配)
- [UART1 调参 + TF/SD 遥测日志](#uart1-调参--tfsd-遥测日志)
...
- `MDK-ARM/`：Keil 工程
- `User/application/`：业务逻辑与任务（底盘/云台/射击/裁判/姿态/检测/调参遥测等）
- `User/bsp/`：板级支持封装（CAN/I2C/SPI/USART/PWM/延时等）
- `User/components/`：通用模块（PID/滤波/CRC/FIFO 等）
...
```
USART3(DBUS) -> remote_control -> behaviour -> chassis_task / gimbal_task
INS(BMI088/IST) -> INS_task -> 角度/角速度/四元数
CAN RX -> CAN_receive -> 电机反馈(measure)

chassis_task/gimbal_task -> actuator_cmd -> can_tx_task -> CAN1(0x200/0x1FF)

USART6(裁判) -> referee_usart_task -> referee.c -> 功率/热量等

USB CDC -> usb_task(vision rx/tx)
UART1 -> usb_task(uart1 tune + UART1 JustFloat telemetry tx + TF/SD sdlog telemetry log)
```
...
- `USART1`（`PA9 TX` / `PB7 RX`）：UART1 端口（根据波特率选择：调参/遥测 `921600 8N1`；ELRS/CRSF 接收 `420000 8N1`）

> UART1 实时遥测（JustFloat）使用 DMA TX（`HAL_UART_Transmit_DMA`），带宽有限需要统一发送周期；TF/SD 会一直记录更详细的运行日志（sdlog）。

### CAN

- **CAN1（指令发送）**：由 `can_tx_task` 统一发送
  - 0x200 → 0x201(M1), 0x202(M2), 0x203(Pitch), 0x204(Trigger)
  - 0x1FF → 0x205(M4), 0x206(Yaw), 0x207(M3), 0x208(预留)
- **CAN2**：工程中预留扩展接口（视具体配置/接线使用）
...
实现位置：`User/application/usb_task.c`

### 1) 调参（UART1 命令）

- 命令格式：`<id>:<value>\r\n`
- `id` 对应 `User/application/app_config.c` 中每行末尾的 `[ID]` 注释
- 只修改 RAM 中的 `g_app_config`；**重启会恢复默认值**

示例（具体 ID 以 `app_config.c` 为准）：

- 开关 UART1 实时遥测：`241:1`（开）/ `241:0`（关）
- UART1 遥测周期(ms)：`242:0`（0=auto，额外50%回退，适合无线）
- UART1 模式判定：波特率为 `420000` 时进入 ELRS/CRSF 接收（不发送遥测）；否则为调参/遥测。
- TF/SD 遥测日志：默认一直记录（只要 TF 挂载且 `sdlog_task` 正常运行），不需要开关/配置。

### 2) UART1 实时遥测（JustFloat：N*fp32 + INF 尾）
...
在 `User/application/app_config.c`：

- `uart1_telem.channel_num = 0`

此时发送 **内置默认列表**（当前 `N=219` 通道；`channel_map[]` 会被忽略），统一周期由 `uart1_telem.period_ms` 控制（0=auto）。

默认列表在“原全量”的基础上精简了 `*_MODE/*_OFFLINE/*_ERR` 等通道，并用 2 个打包通道替代（见下表）；默认列表内容见 `User/application/usb_task.c` 的 `uart1_telem_default_list[]`。

**PACK_MODE（`UART1_TELEM_SIG_PACK_MODE`）**
...
**PACK_OFFLINE（`UART1_TELEM_SIG_PACK_OFFLINE`）**
...
若 UART1 丢包/解析异常：优先增大周期（如 `242:40`）或减少通道数（降低单帧字节数）。

新增板载按键遥测：`UART1_TELEM_SIG_BOARD_KEY_DOWN`（1=按下）、`UART1_TELEM_SIG_BOARD_KEY_PRESS_CNT`（去抖计数）。

#### 自定义遥测列表

把 `uart1_telem.channel_map[]` 填成“遥测信号 ID 列表”，并设置 `uart1_telem.channel_num = <列表长度>`。

- `channel_num=0` 表示使用默认列表（219 通道）
- `0` 是合法的遥测信号 ID（`UART1_TELEM_SIG_SYS_TICK_MS`），不再作为结束符

- 遥测信号枚举：`uart1_telem_sig_e`（见 `User/application/app_config.h`）
...
- PC 解压为 v1 记录流：`python analyzelog/sdlog_decompress.py sdlog_0000.bin`（输出 `sdlog_0000.bin.raw.bin`）
- Web 可视化：`python analyzelog/sdlog_viewer.py sdlog_0000.bin`（浏览器打开地址，可导出 CSV）
- 记录内容：IMU/PID/云台&底盘 loop/CAN/裁判/视觉等（tag 定义见 `User/application/sdlog.h`）
- 注意：TF/SD 不会记录 UART1 的 JustFloat 帧（避免重复/占用写入带宽）

---

## USB CDC 视觉链路

实现位置：`User/application/usb_task.c` + `USB_DEVICE/App/usbd_cdc_if.c`
...
底盘功率限制实现：`User/application/chassis_power_control.c`
...
相关参数：`User/application/app_config.c` 的 `g_app_config.power.*`

---

## 调参日志目录

工程根目录下提供 `analyzelog/logs/` 用于保存调参记录、VOFA 配置、脚本等。

- 该目录内容默认 **不纳入版本管理**（已加入仓库 `.gitignore`）
- 仅保留 `analyzelog/logs/.gitkeep` 用于占位
