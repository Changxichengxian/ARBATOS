...
USB CDC -> host_link_task(vision rx/tx)
AUX 口（当前 HERO 为 USART1） -> host_link_task(AUX 调参/遥测 + TF/SD sdlog telemetry log) + elrs_task.c(ELRS/CRSF 接收)
```
...
实现位置：`shared/application/host_link_task.c`、`shared/application/image_remote_link.c`
...
实现位置：`shared/application/host_link_task.c`、`shared/application/vision_link.c` + `USB_DEVICE/App/usbd_cdc_if.c`
