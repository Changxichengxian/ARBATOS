...
AUX 口（当前 HERO 为 USART1） -> usb_task(AUX 调参/遥测 + TF/SD sdlog telemetry log) + elrs_task.c(ELRS/CRSF 接收)
```
...
- `USART1`（`PA9 TX` / `PB7 RX`）：AUX 口（可换的辅助串口），根据波特率选择：调参/遥测 `921600 8N1`；ELRS/CRSF 接收 `420000 8N1`
...
- 这里说的 AUX 口是可换的辅助串口；当前 HERO 板接在 USART1。配置结构、枚举和值表都使用 `aux_*`。
