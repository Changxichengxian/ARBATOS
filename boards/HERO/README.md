...
- 命令格式：`<id>:<value>\r\n`
- `id` 对应 `User/application/config.c` 中每行末尾的 `[ID]` 注释
...
在 `User/application/config.c`：

- `aux_telem.channel_num = 0`

此时发送 **内置默认列表**（当前 `N=219` 通道；`channel_map[]` 会被忽略），统一周期由 `aux_telem.period_ms` 控制（0=auto）。

默认列表在“原全量”的基础上精简了 `*_MODE/*_OFFLINE/*_ERR` 等通道，并用 2 个打包通道替代（见下表）；默认列表内容见 `User/application/usb_task.c` 的 `aux_telem_default_list[]`。
...
- 遥测信号枚举：`aux_telem_sig_e`（见 `User/application/config.h`）
...
相关参数：`User/application/config.c` 的 `g_config.power.*`
