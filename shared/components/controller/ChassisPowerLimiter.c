...
typedef struct
{
    ...
    fp32 k4;
    fp32 k5;
} ChassisPowerLimiterMotorPowerModel;

static uint8_t ChassisPowerLimiterGetMotorPowerModel(MotorModel model,
                                                           ChassisPowerLimiterMotorPowerModel *out_model);
static uint8_t ChassisPowerLimiterModelCurrentToAmpere(const motor_node_param_t *node,
                                                             fp32 current_cmd,
                                                             fp32 *out_current_a);
static uint8_t ChassisPowerLimiterBuildMotorTerms(const motor_node_param_t *node,
                                                       int16_t wheel_rpm,
                                                       fp32 current_cmd,
                                                       fp32 *out_a,
                                                       fp32 *out_b,
                                                       fp32 *out_c,
                                                       fp32 *out_full_power);
static fp32 ChassisPowerLimiterGetNominalTotalCurrentLimit(const ChassisPowerLimiterConfig *cfg);
static fp32 ChassisPowerLimiterCalcBufferCurrentLimit(const ChassisPowerLimiterConfig *cfg,
                                                            fp32 ChassisPowerBuffer);
static fp32 ChassisPowerLimiterEvalQuadratic(fp32 a, fp32 b, fp32 c, fp32 scale);
static fp32 ChassisPowerLimiterSolveQuadraticScale(fp32 a, fp32 b, fp32 c, fp32 power_limit);

fp32 ChassisPowerLimiterCalcTotalCurrentLimit(const ChassisPowerLimiterConfig *cfg,
                                                    fp32 ChassisPower,
                                                    fp32 ChassisPowerBuffer)
{
    if (cfg == NULL)
    ...
    if (ChassisPowerBuffer < cfg->warning_power_buffer)
    {
        return ChassisPowerLimiterCalcBufferCurrentLimit(cfg, ChassisPowerBuffer);
    }

    if (ChassisPower > cfg->warning_power)
    {
        fp32 power_scale = 0.0f;
        const fp32 denom = (cfg->power_limit - cfg->warning_power);
        if (ChassisPower < cfg->power_limit && denom > 0.0f)
        {
            power_scale = (cfg->power_limit - ChassisPower) / denom;
        }
...
fp32 ChassisPowerLimiterCalcPowerBudget(const ChassisPowerLimiterConfig *cfg,
                                             fp32 ChassisPower,
                                             fp32 ChassisPowerBuffer,
                                             uint8_t buffer_limit_active)
{
    const fp32 nominal_total_current_limit = ChassisPowerLimiterGetNominalTotalCurrentLimit(cfg);
    ...
    {
        total_current_limit = ChassisPowerLimiterCalcBufferCurrentLimit(cfg, ChassisPowerBuffer);
    }
    else
    {
        total_current_limit = ChassisPowerLimiterCalcTotalCurrentLimit(cfg, ChassisPower, ChassisPowerBuffer);
    }
...
uint8_t ChassisPowerLimiterIsPowerModelReady(const motor_node_param_t motor_nodes[4])
{
    ...
    for (uint8_t i = 0u; i < CHASSIS_POWER_LIMITER_MOTOR_COUNT; i++)
    {
        ...
        active_count++;
        entry = MotorModelDbGet(node->model);
        if (entry == NULL || entry->cmd_current_range_abs <= 0 || entry->torque_current_range_a <= 0.0f)
        {
            return 0u;
        }

        if (ChassisPowerLimiterGetMotorPowerModel(node->model, &model) == 0u)
        {
            return 0u;
        }
    }
...
uint8_t ChassisPowerLimiterCalcMotorPower(const motor_node_param_t *node,
                                               int16_t wheel_rpm,
                                               fp32 current_cmd,
                                               fp32 *out_power)
{
    ...
    if (ChassisPowerLimiterBuildMotorTerms(node, wheel_rpm, current_cmd, &a, &b, &c, &full_power) == 0u)
    {
        return 0u;
    }
...
fp32 ChassisPowerLimiterLimitSingleCurrentByPowerModel(const motor_node_param_t *node,
                                                               int16_t wheel_rpm,
                                                               fp32 current_cmd,
                                                               fp32 power_limit,
                                                               fp32 *out_current,
                                                               fp32 *out_power)
{
    ...
    scale = ChassisPowerLimiterSolveQuadraticScale(a, b, c, power_limit);
    if (out_current != NULL)
    {
...
fp32 ChassisPowerLimiterScaleCurrents(fp32 currents[4],
                                                         ...
                                                         fp32 *out_total_power)
{
    ...
    for (uint8_t i = 0u; i < CHASSIS_POWER_LIMITER_MOTOR_COUNT; i++)
    {
        ...
        if (ChassisPowerLimiterModelCurrentToAmpere(node, currents[i], &current_a) == 0u)
        {
            if (out_total_power != NULL)
            {
                *out_total_power = 0.0f;
    ...
    total_power = ChassisPowerLimiterEvalQuadratic(quad_a, quad_b, quad_c, 1.0f);
    if (total_power < 0.0f)
    {
        total_power = 0.0f;
    }
    ...
    {
        const fp32 scale = ChassisPowerLimiterSolveQuadraticScale(quad_a, quad_b, quad_c, power_limit);
        for (uint8_t i = 0u; i < CHASSIS_POWER_LIMITER_MOTOR_COUNT; i++)
        {
            currents[i] *= scale;
        }
                                                       ...
                                                       fp32 *out_full_power)
{
    ...
    if (ChassisPowerLimiterModelCurrentToAmpere(node, current_cmd, &current_a) == 0u)
    {
        return 0u;
    }
    ...
    full_power = ChassisPowerLimiterEvalQuadratic(a, b, c, 1.0f);
                                                            ...
                                                            fp32 ChassisPowerBuffer)
{
    ...
    if (denom > 0.0f)
    {
        if (ChassisPowerBuffer >= denom)
        {
            power_scale = 1.0f;
        }
        else if (ChassisPowerBuffer > CHASSIS_POWER_LIMITER_MIN_BUFFER_J)
        {
            power_scale = ChassisPowerBuffer / denom;
        }
        else
        {
...
static fp32 ChassisPowerLimiterSolveQuadraticScale(fp32 a, fp32 b, fp32 c, fp32 power_limit)
{
    const fp32 base_power = ChassisPowerLimiterEvalQuadratic(a, b, c, 0.0f);
    const fp32 full_power = ChassisPowerLimiterEvalQuadratic(a, b, c, 1.0f);
