                                             ...
                                             uint8_t safe_pos)
{
    ...
    const uint16_t follow_raw = (uint16_t)input_switch_pos_to_raw(follow_pos);
    ...
    if (manual_online == 0u)
    {
        gate_inited = 0u;
        down_engaged = 0u;
        last_sw_raw = (uint16_t)input_switch_pos_to_raw(safe_pos);
        return raw_sw;
    }
    ...
    if (!input_switch_is_pos(raw_sw, spin_pos))
    {
        down_engaged = 0u;
    }
    else if (!input_switch_is_pos(last_sw_raw, spin_pos))
    {
        down_engaged = 1u;
    }

    if (input_switch_is_pos(raw_sw, spin_pos) && down_engaged == 0u)
    {
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    if (input_switch_is_pos(chassis_sw, fast->safe_pos))
    {
    ...
    else if (input_switch_is_pos(chassis_sw_effective, fast->follow_pos))
    {
    ...
    else if (input_switch_is_pos(chassis_sw_effective, fast->spin_pos))
    {
        // 小陀螺：持续自转（类似 WH 的 SelfProtect），同时保留平移控制
        chassis_behaviour_mode = ((fast->key_mask & fast->gyro_spin_var_key_mask) != 0u) ?
...
static void chassis_engineer_follow_chassis_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    *angle_set = rad_format(chassis_move_rc_to_vector->chassis_yaw_set -
                            chassis_move_rc_to_vector->fast.angle_z_rc_sen * chassis_move_rc_to_vector->fast.axis_wz);
}
...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
    *wz_set = -chassis_move_rc_to_vector->fast.wz_rc_sen * chassis_move_rc_to_vector->fast.axis_wz;
}
...
static void chassis_open_set_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    *vx_set = chassis_move_rc_to_vector->fast.axis_x * chassis_move_rc_to_vector->fast.open_rc_scale;
    *vy_set = -chassis_move_rc_to_vector->fast.axis_y * chassis_move_rc_to_vector->fast.open_rc_scale;
