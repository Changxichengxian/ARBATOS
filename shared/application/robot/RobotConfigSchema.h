...
} RobotDeviceConfigEntry;

typedef struct
{
    uint8_t count;
    RobotDeviceConfigEntry entry[ROBOT_DEVICE_CONFIG_MAX];
} RobotDeviceConfigTable;

#define ROBOT_DEVICE_CONFIG_OFFSET(member_) ((uint32_t)offsetof(Config, member_))

#define ROBOT_DEVICE_ENTRY_RAW(name_, kind_, role_, role_index_, fallback_bus_, source_id_, ConfigOffset) \
    {(name_), (uint32_t)(ConfigOffset), (uint16_t)(source_id_), (kind_), (role_), (role_index_), (fallback_bus_)}

#define ROBOT_DEVICE_ENTRY(name_, kind_, role_, role_index_, fallback_bus_, source_id_, ConfigMember) \
    ROBOT_DEVICE_ENTRY_RAW((name_), (kind_), (role_), (role_index_), (fallback_bus_), (source_id_), ROBOT_DEVICE_CONFIG_OFFSET(ConfigMember))
...
#define ROBOT_DEVICE_ENTRY_MOTOR(name_, role_, role_index_, fallback_bus_, source_id_, ConfigMember) \
    ROBOT_DEVICE_ENTRY((name_), ROBOT_DEVICE_TABLE_KIND_MOTOR, (role_), (role_index_), (fallback_bus_), (source_id_), ConfigMember)

#define ROBOT_DEVICE_ENTRY_SENSOR(name_, role_, role_index_, ConfigMember) \
    ROBOT_DEVICE_ENTRY((name_), ROBOT_DEVICE_TABLE_KIND_SENSOR, (role_), (role_index_), 0u, ROBOT_DEVICE_SOURCE_NONE, ConfigMember)

#define ROBOT_DEVICE_ENTRY_INPUT(name_, role_, role_index_, ConfigMember) \
    ROBOT_DEVICE_ENTRY((name_), ROBOT_DEVICE_TABLE_KIND_INPUT, (role_), (role_index_), 0u, ROBOT_DEVICE_SOURCE_NONE, ConfigMember)

#define ROBOT_DEVICE_ENTRY_COMM(name_, role_, role_index_, ConfigMember) \
    ROBOT_DEVICE_ENTRY((name_), ROBOT_DEVICE_TABLE_KIND_COMM, (role_), (role_index_), 0u, ROBOT_DEVICE_SOURCE_NONE, ConfigMember)

#define ROBOT_DEVICE_ENTRY_SERVICE(name_, role_, role_index_, ConfigMember) \
    ROBOT_DEVICE_ENTRY((name_), ROBOT_DEVICE_TABLE_KIND_SERVICE, (role_), (role_index_), 0u, ROBOT_DEVICE_SOURCE_NONE, ConfigMember)
