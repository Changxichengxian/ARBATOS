...
} arm_core_joint_role_e;

typedef struct
{
    uint8_t enabled;
    uint8_t role; // arm_core_joint_role_e
    int8_t direction;
    uint16_t key_mask;
    fp32 key_speed_rad_s;
    fp32 max_kd;
} arm_core_joint_param_t;
...
static inline void arm_core_step_manual(const arm_core_config_t *cfg,
                                        const arm_core_joint_param_t *joint,
                                        uint8_t joint_count,
                                        const arm_core_input_t *input,
                                        arm_core_output_t *output)
{
    uint8_t i;
    uint8_t count;
    uint8_t any_key = 0u;
    ...
    for (i = 0u; i < count; i++)
    {
        if (arm_core_key_active(&joint[i], input->key_mask) == 0u)
        {
            continue;
        }
    ...
    for (i = 0u; i < count; i++)
    {
        const arm_core_joint_param_t *param = &joint[i];
        const uint8_t move_key = arm_core_key_active(param, input->key_mask);
