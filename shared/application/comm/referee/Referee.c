 ...
 * - 后段：RefereeDataSolve() 解析裁判帧，getter 提供功率/热量/机器人状态。
 * - 发送：RefereeSendFrame() 统一补帧头、CRC，并交给 USART。
 */
...
frame_header_struct_t RefereeReceiveHeader;
frame_header_struct_t RefereeSendHeader;
...
static void RefereeUiSetName(uint8_t dst[3], const char name[3]);
static void RefereeUiInitGraphic(RefereeUiGraphic *graphic,
                                    const char name[3],
                                    uint8_t operation,
                                    uint8_t type,
                                    uint8_t layer,
                                    ...
                                    uint16_t start_x,
                                    uint16_t start_y);
static void RefereeUiSetNumberBits(RefereeUiGraphic *graphic, int32_t value);
static uint8_t RefereeUiUtf8ToUtf16le(const char *text_utf8,
                                          uint8_t out_utf16[REFEREE_UI_STRING_UTF16_BYTES],
                                          uint16_t *out_code_units);

uint16_t RefereeGetClientIdByRobotId(uint16_t robot_id)
{
...
uint16_t RefereeGetSelfClientId(void)
{
    return RefereeGetClientIdByRobotId((uint16_t)get_robot_id());
}

void RefereeUiMakeLine(RefereeUiGraphic *graphic,
                          const char name[3],
                          uint8_t operation,
                          uint8_t layer,
                          uint8_t color,
                          ...
                          uint16_t end_y)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_LINE, layer, color, width, start_x, start_y);
    graphic->details_d = end_x;
    graphic->details_e = end_y;
}

void RefereeUiMakeRect(RefereeUiGraphic *graphic,
                          const char name[3],
                          uint8_t operation,
                          uint8_t layer,
                          uint8_t color,
                          ...
                          uint16_t end_y)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_RECT, layer, color, width, start_x, start_y);
    graphic->details_d = end_x;
    graphic->details_e = end_y;
}

void RefereeUiMakeCircle(RefereeUiGraphic *graphic,
                            const char name[3],
                            uint8_t operation,
                            uint8_t layer,
                            uint8_t color,
                            ...
                            uint16_t radius)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_CIRCLE, layer, color, width, center_x, center_y);
    graphic->details_c = radius;
}

void RefereeUiMakeEllipse(RefereeUiGraphic *graphic,
                             const char name[3],
                             uint8_t operation,
                             uint8_t layer,
                             uint8_t color,
                             ...
                             uint16_t y_half_axis)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_ELLIPSE, layer, color, width, center_x, center_y);
    graphic->details_d = x_half_axis;
    graphic->details_e = y_half_axis;
}

void RefereeUiMakeArc(RefereeUiGraphic *graphic,
                         const char name[3],
                         uint8_t operation,
                         uint8_t layer,
                         uint8_t color,
                         ...
                         uint16_t y_half_axis)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_ARC, layer, color, width, center_x, center_y);
...
void RefereeUiMakeInt(RefereeUiGraphic *graphic,
                         const char name[3],
                         uint8_t operation,
                         uint8_t layer,
                         uint8_t color,
                         ...
                         int32_t value)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_INT, layer, color, width, start_x, start_y);
    graphic->details_a = font_size;
    RefereeUiSetNumberBits(graphic, value);
}

void RefereeUiMakeFloat(RefereeUiGraphic *graphic,
                           const char name[3],
                           uint8_t operation,
                           uint8_t layer,
                           uint8_t color,
                           ...
                           fp32 value)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_FLOAT, layer, color, width, start_x, start_y);
    graphic->details_a = font_size;
    RefereeUiSetNumberBits(graphic, int_value);
}

void RefereeUiMakeChar(RefereeUiGraphic *graphic,
                          const char name[3],
                          uint8_t operation,
                          uint8_t layer,
                          uint8_t color,
                          ...
                          uint16_t font_size)
{
    ...
    RefereeUiInitGraphic(graphic, name, operation, REFEREE_UI_CHAR, layer, color, width, start_x, start_y);
    graphic->details_a = font_size;
}

int RefereeUiDelete(uint16_t receiver_id, uint8_t delete_type, uint8_t layer)
{
    uint8_t payload[2] = {0};
    ...
    payload[1] = layer;
    return RefereeSendStudentInteractive((uint16_t)REFEREE_UI_DATA_DELETE, receiver_id, payload, (uint16_t)sizeof(payload));
}

int RefereeUiDeleteSelf(uint8_t delete_type, uint8_t layer)
{
    return RefereeUiDelete(RefereeGetSelfClientId(), delete_type, layer);
}

int RefereeUiSendGraphics(uint16_t receiver_id, const RefereeUiGraphic *graphics, uint8_t count)
{
    uint8_t payload[REFEREE_UI_GRAPHIC_RAW_LEN * 7u] = {0};
    uint16_t data_cmd_id = RefereeUiCountToDataCmdId(count);
    uint8_t i = 0u;
    ...
    for (i = 0u; i < count; i++)
    {
        RefereeUiPackGraphic(&graphics[i], &payload[(uint16_t)i * REFEREE_UI_GRAPHIC_RAW_LEN]);
    }

    return RefereeSendStudentInteractive(data_cmd_id,
                                            receiver_id,
                                            payload,
                                            (uint16_t)count * REFEREE_UI_GRAPHIC_RAW_LEN);
}

int RefereeUiSendGraphicsSelf(const RefereeUiGraphic *graphics, uint8_t count)
{
    return RefereeUiSendGraphics(RefereeGetSelfClientId(), graphics, count);
}

int RefereeUiSendStringUtf8(uint16_t receiver_id, const RefereeUiGraphic *graphic, const char *text_utf8)
{
    RefereeUiGraphic graphic_copy;
    ...
    (void)RefereeUiUtf8ToUtf16le(text_utf8, &payload[REFEREE_UI_GRAPHIC_RAW_LEN], &code_units);
    graphic_copy.details_b = code_units;
    RefereeUiPackGraphic(&graphic_copy, payload);

    return RefereeSendStudentInteractive((uint16_t)REFEREE_UI_DATA_CHAR,
                                            receiver_id,
                                            payload,
                                            (uint16_t)sizeof(payload));
}

int RefereeUiSendStringUtf8Self(const RefereeUiGraphic *graphic, const char *text_utf8)
{
    return RefereeUiSendStringUtf8(RefereeGetSelfClientId(), graphic, text_utf8);
}

void RefereeUiDemoTick(void)
{
    ...
    static RefereeUiGraphic demo_graphics[5];
    static RefereeUiGraphic demo_text_graphic;
    uint16_t client_id = RefereeGetSelfClientId();
    ...
    switch (stage)
    {
        case 0:
        {
            if (RefereeUiDelete(client_id, (uint8_t)REFEREE_UI_DELETE_LAYER, REFEREE_UI_DEMO_LAYER) >= 0)
            {
                last_send_ms = now_ms;
                stage = 1u;
            }
        ...
        case 1:
        {
            ...
            RefereeUiMakeRect(&demo_graphics[0], "U0A", (uint8_t)REFEREE_UI_OP_ADD, REFEREE_UI_DEMO_LAYER,
                                 (uint8_t)REFEREE_UI_COLOR_GREEN, 3u, 860u, 460u, 1060u, 620u);
            RefereeUiMakeLine(&demo_graphics[1], "U0B", (uint8_t)REFEREE_UI_OP_ADD, REFEREE_UI_DEMO_LAYER,
                                 (uint8_t)REFEREE_UI_COLOR_CYAN, 2u, 900u, 540u, 1020u, 540u);
            RefereeUiMakeLine(&demo_graphics[2], "U0C", (uint8_t)REFEREE_UI_OP_ADD, REFEREE_UI_DEMO_LAYER,
                                 (uint8_t)REFEREE_UI_COLOR_CYAN, 2u, 960u, 480u, 960u, 600u);
            RefereeUiMakeCircle(&demo_graphics[3], "U0D", (uint8_t)REFEREE_UI_OP_ADD, REFEREE_UI_DEMO_LAYER,
                                   (uint8_t)REFEREE_UI_COLOR_YELLOW, 2u, 960u, 540u, 80u);
            RefereeUiMakeLine(&demo_graphics[4], "U0E", (uint8_t)REFEREE_UI_OP_ADD, REFEREE_UI_DEMO_LAYER,
                                 (uint8_t)REFEREE_UI_COLOR_ORANGE, 2u, 900u, 500u, 1020u, 580u);

            if (RefereeUiSendGraphics(client_id, demo_graphics, 5u) >= 0)
            {
                last_send_ms = now_ms;
                stage = 2u;
            }
        ...
        case 2:
        {
            ...
            RefereeUiMakeChar(&demo_text_graphic, "TXT", (uint8_t)REFEREE_UI_OP_ADD, REFEREE_UI_DEMO_LAYER,
            ...
            if (RefereeUiSendStringUtf8(client_id, &demo_text_graphic, text) >= 0)
            {
                last_send_ms = now_ms;
                stage = 3u;
            }
        ...
        default:
        {
            ...
            RefereeUiMakeChar(&demo_text_graphic, "TXT", (uint8_t)REFEREE_UI_OP_MODIFY, REFEREE_UI_DEMO_LAYER,
                                 (uint8_t)REFEREE_UI_COLOR_WHITE, 2u, 780u, 680u, 20u);
            snprintf(text,
                     sizeof(text),
                     "B%u H%u/%u",
...
static void RefereeUiPackGraphic(const RefereeUiGraphic *graphic, uint8_t out[REFEREE_UI_GRAPHIC_RAW_LEN])
{
    ...
    RefereeUiStoreU32Le(&out[3], config1);
    RefereeUiStoreU32Le(&out[7], config2);
    RefereeUiStoreU32Le(&out[11], config3);
}
                                    ...
                                    uint16_t start_y)
{
    ...
    RefereeUiSetName(graphic->name, name);
...
{
    memset(&RefereeReceiveHeader, 0, sizeof(frame_header_struct_t));
    memset(&RefereeSendHeader, 0, sizeof(frame_header_struct_t));
    ...
    memset(&RefereeWarning, 0, sizeof(ext_referee_warning_t));
    ...
    memset(&ShootData, 0, sizeof(ext_shoot_data_t));
    ...
    RefereeTxSeq = 0u;
    memset(RefereeTxFrameBuf, 0, sizeof(RefereeTxFrameBuf));
...
{
    return robot_state.ChassisPowerLimit;
}
...
uint8_t RefereeTxReady(void)
{
    return BspRefereeTxReady();
}

int RefereeSendStudentInteractive(uint16_t data_cmd_id,
...
static int RefereeSendFrame(uint16_t cmd_id, const uint8_t *payload, uint16_t payload_len)
{
    ...
    if (frame_len > (uint16_t)sizeof(RefereeTxFrameBuf))
    {
        return -1;
    }

    RefereeTxFrameBuf[0] = HEADER_SOF;
    RefereeTxFrameBuf[1] = (uint8_t)(payload_len & 0x00FFu);
    RefereeTxFrameBuf[2] = (uint8_t)((payload_len >> 8) & 0x00FFu);
    RefereeTxFrameBuf[3] = RefereeTxSeq++;
    RefereeTxFrameBuf[4] = 0u;
    append_CRC8_check_sum(RefereeTxFrameBuf, REF_PROTOCOL_HEADER_SIZE);

    index = REF_PROTOCOL_HEADER_SIZE;
    RefereeTxFrameBuf[index++] = (uint8_t)(cmd_id & 0x00FFu);
    RefereeTxFrameBuf[index++] = (uint8_t)((cmd_id >> 8) & 0x00FFu);

    if (payload != NULL && payload_len != 0u)
    {
        memcpy(&RefereeTxFrameBuf[index], payload, payload_len);
        index = (uint16_t)(index + payload_len);
    }

    RefereeTxFrameBuf[index++] = 0u;
    RefereeTxFrameBuf[index++] = 0u;
    append_CRC16_check_sum(RefereeTxFrameBuf, index);
    return BspRefereeTx(RefereeTxFrameBuf, index);
}
