...
typedef struct
{
    ...
    fp32 y_m;
    fp32 length_m;
} WheelLegCoreFootPoint;

typedef struct
{
    fp32 x_m;
    fp32 v_mps;
} WheelLegCoreObserver;
...
static inline fp32 WheelLegCoreClamp(fp32 value, fp32 min_value, fp32 max_value)
{
    if (value < min_value)
    {
        return min_value;
...
static inline fp32 WheelLegCoreAbs(fp32 value)
{
    return (value >= 0.0f) ? value : -value;
}

static inline uint8_t WheelLegCoreAxisInDeadband(int16_t axis, uint16_t deadband)
{
    return (axis > -(int16_t)deadband && axis < (int16_t)deadband) ? 1u : 0u;
}

static inline fp32 WheelLegCoreAxisToFp32(int16_t axis,
                                             fp32 max_abs,
                                             uint16_t deadband,
                                             int16_t axis_abs)
{
    if (axis_abs <= 0 || WheelLegCoreAxisInDeadband(axis, deadband) != 0u)
    {
        return 0.0f;
    }
    return WheelLegCoreClamp(((fp32)axis) / (fp32)axis_abs, -1.0f, 1.0f) * max_abs;
}

static inline fp32 WheelLegCoreTargetThetaFromFootX(fp32 foot_x_m, fp32 leg_length_m)
{
    fp32 ratio;
    ...
    ratio = WheelLegCoreClamp(foot_x_m / leg_length_m, -0.98f, 0.98f);
    return asinf(ratio);
}

static inline fp32 WheelLegCoreSlewFp32(fp32 current, fp32 target, fp32 max_delta)
{
...
static inline fp32 WheelLegCoreDirSign(int8_t dir)
{
    return (dir < 0) ? -1.0f : 1.0f;
}

static inline fp32 WheelLegCoreJointToRaw(fp32 kinematic_position, fp32 zero_position, int8_t dir)
{
    return zero_position + kinematic_position * WheelLegCoreDirSign(dir);
}

static inline fp32 WheelLegCoreRawToKinematic(fp32 raw_position,
                                                 fp32 raw_zero_position,
                                                 fp32 kinematic_zero_position,
                                                 int8_t dir)
{
    return kinematic_zero_position +
           (raw_position - raw_zero_position) * WheelLegCoreDirSign(dir);
}

static inline fp32 WheelLegCoreKinematicToRaw(fp32 kinematic_position,
                                                 fp32 raw_zero_position,
                                                 fp32 kinematic_zero_position,
                                                 int8_t dir)
{
    return raw_zero_position +
           (kinematic_position - kinematic_zero_position) * WheelLegCoreDirSign(dir);
}

static inline fp32 WheelLegCoreWrapPi(fp32 value)
{
...
static inline fp32 WheelLegCoreNearAngle(fp32 value, fp32 reference)
{
    return reference + WheelLegCoreWrapPi(value - reference);
}

static inline fp32 WheelLegCoreLerp(fp32 start, fp32 end, uint32_t elapsed_ms, uint32_t duration_ms)
{
    fp32 u;
...
static inline fp32 WheelLegCorePoly4(const fp32 coe[WHEELLEG_CORE_LQR_COEFF_COUNT], fp32 x)
{
    if (coe == NULL)
    {
        return 0.0f;
...
static inline uint8_t WheelLegCoreLqrRowIsZero(const fp32 coe[WHEELLEG_CORE_LQR_COEFF_COUNT])
{
    return (coe == NULL ||
...
static inline fp32 WheelLegCoreLqrXError(fp32 observer_x_m,
                                            fp32 target_v_mps,
                                            fp32 target_yaw_rate_radps,
                                            fp32 hold_limit_m,
                                            fp32 motion_eps)
{
    if (WheelLegCoreAbs(target_v_mps) > motion_eps ||
        WheelLegCoreAbs(target_yaw_rate_radps) > motion_eps)
    {
        return 0.0f;
    }
    return WheelLegCoreClamp(observer_x_m, -hold_limit_m, hold_limit_m);
}

static inline fp32 WheelLegCoreLqrWheelOutput(const fp32 k[WHEELLEG_CORE_LQR_ROW_COUNT],
                                                  fp32 theta_err,
                                                  fp32 d_theta,
                                                  fp32 x_err,
                                                  fp32 v_err,
...
static inline fp32 WheelLegCoreLqrHipOutput(const fp32 k[WHEELLEG_CORE_LQR_ROW_COUNT],
                                                fp32 theta_err,
                                                fp32 d_theta,
                                                fp32 x_err,
                                                fp32 v_err,
...
static inline fp32 WheelLegCoreTurnTorque(fp32 yaw_set,
                                             fp32 yaw,
                                             fp32 yaw_gyro,
                                             fp32 kp,
                                             fp32 kd,
                                             fp32 max_out)
{
    const fp32 out = kp * WheelLegCoreWrapPi(yaw_set - yaw) - kd * yaw_gyro;

    return WheelLegCoreClamp(out, -max_out, max_out);
}

static inline fp32 WheelLegCoreRollForce(fp32 roll_set,
                                            fp32 roll,
                                            fp32 roll_gyro,
                                            fp32 kp,
                                            fp32 kd,
...
static inline void WheelLegCorePidConfigure(WheelLegCorePid *pid,
                                               fp32 kp,
                                               fp32 ki,
                                               fp32 kd,
                                               fp32 max_out,
...
static inline void WheelLegCorePidClear(WheelLegCorePid *pid)
{
    if (pid == NULL)
    {
        return;
...
static inline fp32 WheelLegCorePidCalc(WheelLegCorePid *pid, fp32 ref, fp32 set)
{
    fp32 err;
    fp32 out;
    ...
    err = set - ref;
    pid->iout += pid->ki * err;
    pid->iout = WheelLegCoreClamp(pid->iout, -pid->max_iout, pid->max_iout);
    out = pid->kp * err + pid->iout + pid->kd * (err - pid->last_err);
    pid->last_err = err;
    return WheelLegCoreClamp(out, -pid->max_out, pid->max_out);
}

static inline void WheelLegCoreObserverUpdate(WheelLegCoreObserver *observer,
                                                 const WheelLegCoreLegCalc *left_leg,
                                                 const WheelLegCoreLegCalc *right_leg,
                                                 ...
                                                 fp32 dt)
{
    ...
    lpf = WheelLegCoreClamp(lpf, 0.01f, 1.0f);
...
static inline uint8_t WheelLegCoreLimitFootXy(fp32 min_leg_m,
                                                  fp32 max_leg_m,
                                                  ...
                                                  fp32 *length_m)
{
    ...
    y = WheelLegCoreClamp(*y_m, min_leg_m, max_leg_m);
    ...
    x = WheelLegCoreClamp(*x_m, -max_abs_x, max_abs_x);
...
static inline void WheelLegCoreTargetSmoothClear(WheelLegCoreTargetSmooth *smooth)
{
    if (smooth == NULL)
    {
        return;
...
static inline uint8_t WheelLegCoreTargetSmoothUpdateXy(WheelLegCoreTargetSmooth *smooth,
                                                            ...
                                                            fp32 dt)
{
    ...
    if (WheelLegCoreLimitFootXy(min_leg_m,
                                    max_leg_m,
                                    max_foot_x_range_m,
                                    &target_foot_x_m,
                                    &target_foot_y_m,
    ...
    if (smooth->valid == 0u)
    {
        measured_alpha_rad = WheelLegCoreClamp(measured_alpha_rad, -1.2f, 1.2f);
        measured_length = WheelLegCoreClamp(measured_leg_m, min_leg_m, max_leg_m);
    ...
    smooth->foot_x_m = WheelLegCoreSlewFp32(smooth->foot_x_m,
                                               target_foot_x_m,
                                               foot_x_slew_mps * dt);
    smooth->foot_y_m = WheelLegCoreSlewFp32(smooth->foot_y_m,
                                               target_foot_y_m,
                                               foot_y_slew_mps * dt);
    smooth->length_m = target_length;
    return WheelLegCoreLimitFootXy(min_leg_m,
                                       max_leg_m,
                                       max_foot_x_range_m,
                                       &smooth->foot_x_m,
                                       &smooth->foot_y_m,
...
static inline uint8_t WheelLegCoreForwardPoint(const WheelLegCoreGeometry *geo,
                                                  fp32 front_pos,
                                                  fp32 back_pos,
                                                  WheelLegCoreFootPoint *out)
{
    ...
    if (discr < -0.000001f || WheelLegCoreAbs(a0 + c0) < 0.000001f)
    {
        return 0u;
    }
    if (discr < 0.0f)
...
static inline uint8_t WheelLegCoreCalcKinematics(const WheelLegCoreGeometry *geo,
                                                    WheelLegCoreLegCalc *leg,
                                                    fp32 front_pos,
                                                    fp32 back_pos,
                                                    fp32 pitch,
                                                    fp32 gyro_y,
...
static inline uint8_t WheelLegCoreCalcVmc(WheelLegCoreLegCalc *leg)
{
    fp32 sin32;
    fp32 j11;
    fp32 j12;
    ...
    if (WheelLegCoreAbs(sin32) < 0.0001f)
    {
        return 0u;
    }
...
static inline uint8_t WheelLegCoreInversePoint(const WheelLegCoreGeometry *geo,
                                                  const WheelLegCoreFootPoint *target,
                                                  fp32 front_ref,
                                                  fp32 back_ref,
                                                  fp32 *front_out,
                                                  fp32 *back_out)
{
    ...
    front_cos = WheelLegCoreClamp(front_cos, -1.0f, 1.0f);
    back_cos = WheelLegCoreClamp(back_cos, -1.0f, 1.0f);
    front_base = atan2f(cy, cx);
    back_base = atan2f(cy, back_dx);
    front_q = acosf(front_cos);
    back_q = acosf(back_cos);

    front_raw[0] = WheelLegCoreNearAngle(front_base + front_q - WHEELLEG_CORE_PI * 0.5f, front_ref);
    front_raw[1] = WheelLegCoreNearAngle(front_base - front_q - WHEELLEG_CORE_PI * 0.5f, front_ref);
    back_raw[0] = WheelLegCoreNearAngle(back_base + back_q - WHEELLEG_CORE_PI * 0.5f, back_ref);
    back_raw[1] = WheelLegCoreNearAngle(back_base - back_q - WHEELLEG_CORE_PI * 0.5f, back_ref);

    for (i = 0u; i < 2u; i++)
    {
        for (j = 0u; j < 2u; j++)
        {
            WheelLegCoreFootPoint check;
            fp32 err;
            fp32 score;

            if (WheelLegCoreForwardPoint(geo, front_raw[i], back_raw[j], &check) == 0u)
            {
                continue;
            }
            err = WheelLegCoreAbs(check.x_m - target->x_m) + WheelLegCoreAbs(check.y_m - target->y_m);
            if (err > 0.003f)
            {
                continue;
            }

            score = WheelLegCoreAbs(WheelLegCoreWrapPi(front_raw[i] - front_ref)) +
                    WheelLegCoreAbs(WheelLegCoreWrapPi(back_raw[j] - back_ref)) +
                    err * 1000.0f;
...
static inline void WheelLegCoreSetJointTorque(WheelLegCoreOutput *out,
                                                  WheelLegCoreActuator actuator,
                                                  fp32 torque_nm)
{
...
static inline void WheelLegCoreSetStateTorque(WheelLegCoreOutput *out,
                                                  WheelLegCoreActuator actuator,
                                                  fp32 position_rad,
                                                  fp32 velocity_radps,
                                                  fp32 kp,
...
static inline void WheelLegCoreSetWheelTorques(WheelLegCoreOutput *out,
                                                   const fp32 wheel_torque_nm[WHEELLEG_SIDE_COUNT])
{
    ...
    WheelLegCoreSetJointTorque(out,
                                   WHEELLEG_CORE_ACT_RIGHT_WHEEL,
...
static inline void WheelLegCoreSetVmcJointTorques(WheelLegCoreOutput *out,
                                                       const WheelLegCoreLegCalc leg[WHEELLEG_SIDE_COUNT],
                                                       ...
                                                       int8_t left_back_dir)
{
    ...
    WheelLegCoreSetJointTorque(out,
                                   ...
                                   right_leg->joint_torque[(uint8_t)WHEELLEG_CORE_JOINT_FRONT] *
                                       WheelLegCoreDirSign(right_front_dir));
    WheelLegCoreSetJointTorque(out,
                                   WHEELLEG_CORE_ACT_RIGHT_BACK,
                                   right_leg->joint_torque[(uint8_t)WHEELLEG_CORE_JOINT_BACK] *
                                       WheelLegCoreDirSign(right_back_dir));
    WheelLegCoreSetJointTorque(out,
                                   WHEELLEG_CORE_ACT_LEFT_FRONT,
                                   left_leg->joint_torque[(uint8_t)WHEELLEG_CORE_JOINT_FRONT] *
                                       WheelLegCoreDirSign(left_front_dir));
    WheelLegCoreSetJointTorque(out,
                                   WHEELLEG_CORE_ACT_LEFT_BACK,
                                   left_leg->joint_torque[(uint8_t)WHEELLEG_CORE_JOINT_BACK] *
                                       WheelLegCoreDirSign(left_back_dir));
}

static inline void WheelLegCoreOutputClear(WheelLegCoreOutput *out)
{
    if (out == NULL)
    {
        return;
