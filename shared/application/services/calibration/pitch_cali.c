...
static pitch_cali_table_t s_boot_table_tmp;

__weak bool_t pitch_cali_get_builtin_default(pitch_cali_builtin_desc_t *out)
{
    if (out != NULL)
    {
...
static bool_t pitch_cali_load_builtin_default(pitch_cali_table_t *out);
...
static bool_t pitch_cali_load_builtin_default(pitch_cali_table_t *out)
{
    pitch_cali_builtin_desc_t builtin = {0};
    uint16_t gridN = 0u;
    ...
    if (pitch_cali_get_builtin_default(&builtin) == 0)
    {
        return 0;
    }
    ...
    gridN = pitch_cali_grid_count_from_dims(builtin.angle_points, builtin.bullet_points);
...
void pitch_cali_boot_load(void)
{
    ...
    (void)pitch_cali_load_builtin_default(&s_ctx.table);
    ...
    if (pitch_cali_load_from_sd(&s_boot_table_tmp) == 0)
    {
