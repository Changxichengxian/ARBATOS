...
#define imu_start_buzzer()          BuzzerToneStartLegacy(g_config.buzzer.imu_cali_psc, g_config.buzzer.imu_cali_pwm)
//when gimbal is calibrating ,buzzer set frequency and strength.当云台在校准,蜂鸣器的设置频率和强度
#define GimbalStartBuzzer()       BuzzerToneStartLegacy(g_config.buzzer.GimbalCaliPsc, g_config.buzzer.GimbalCaliPwm)
#define cali_buzzer_off()           BuzzerToneStop()            //buzzer off，关闭蜂鸣器
...
#define rc_cali_buzzer_middle_on()  GimbalStartBuzzer()
...
extern bool_t CalibrateGyroOffsetSave(const fp32 offset[3]);
...
/**
  ...
  * @retval         none
  */
extern void CalibrateTask(void const *pvParameters);
