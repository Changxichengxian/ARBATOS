...
static inline void DetectCommonHook(DetectError *list,
                                      uint32_t *last_tick_ms,
                                      uint8_t count,
                                      uint8_t toe,
                                      uint32_t now_ms)
...
static inline bool_t DetectCommonIsError(DetectError *list,
                                             ...
                                             uint32_t now_ms)
{
    ...
    for (uint8_t i = 0u; i < count; i++)
    {
        (void)DetectCommonIsError(list, last_tick_ms, count, i, now_ms);
    }
}
