...
static uint16_t shoot_get_raw_switch(void)
{
    ...
    uart1_image_remote_state_t image_state;
    if (!uart1_image_remote_get_state(&image_state) ||
...
static void shoot_entertain_music_control(test_mode_e mode)
{
    ...
    if (switch_is_mid(shoot_sw))
    {
        want_key = 1u;
        want_name = pcm_cfg->mid_file;
    }
    else if (switch_is_down(shoot_sw))
    {
        want_key = 2u;
...
int16_t shoot_control_loop(void)
{
    ...
    const uint16_t shoot_sw = shoot_get_effective_switch();
    const bool_t sw_mid = switch_is_mid(shoot_sw);
    const bool_t sw_down = switch_is_down(shoot_sw);
    ...
    if(shoot_control.shoot_mode == SHOOT_STOP)
    {
        shoot_laser_off();
        shoot_control.given_current = 0;
        // 上档/停火：摩擦轮直接下发 0 电流，避免速度环制动产生再生电流
        shoot_clear_fric_output();
    }
    else
    {
        ...
        // 摩擦轮目标转速斜坡：平滑起转/提速
        const fp32 fric_ramp_step = sw_mid ? (SHOOT_FRIC_SPEED_STEP_RPM_S * 0.5f) : SHOOT_FRIC_SPEED_STEP_RPM_S;
        ramp_calc(&shoot_control.fric_speed_ramp, fric_ramp_step);

    }

    if(!allow_trigger || !sw_down)
    {
        shoot_clear_trigger_output();
    }
    ...
    if (allow_fric && shoot_control.shoot_mode != SHOOT_STOP)
    {
        ...
        // 原子性更新四路电流，避免同一帧 CAN 内出现“混合的新旧指令”。
        taskENTER_CRITICAL();
        for (uint8_t i = 0; i < FRIC_MOTOR_NUM; i++)
        {
...
static void shoot_set_mode(void)
{
    ...
    if (switch_is_up(shoot_sw))
    {
        shoot_control.shoot_mode = SHOOT_STOP;
    }
    else if (switch_is_mid(shoot_sw))
    {
    ...
    if(sw_down && shoot_control.shoot_mode == SHOOT_READY_FRIC && (fric_ready || (!allow_fric && allow_trigger)))
    {
...
static void shoot_feedback_update(void)
{
    ...
    if (uart1_image_aux_fire_requested())
    {
    ...
    //射击开关下档时间计时
    if (shoot_control.shoot_mode != SHOOT_STOP && switch_is_down(shoot_get_effective_switch()))
    {
