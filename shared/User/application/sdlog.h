...
typedef enum
{
    ...
    SDLOG_TAG_APP_WATCH = 0x0031u,      // payload: app_watch_t (see app_watch.h)
    ...
    // Vision over USB CDC (PC -> board)
    SDLOG_TAG_VISION_USB_RX = 0x0043u, // payload: sdlog_vision_to_gimbal_t (same as VisionToGimbal)

    // UART1 tuning command (ASCII line)
    SDLOG_TAG_UART1_TUNE = 0x0044u, // payload: uint32_t seq + NUL-terminated command string
...
typedef struct __attribute__((packed))
{
    ...
    uint8_t rc_ctrl[22]; // RC_ctrl_t (packed) snapshot for exact control values used by the system
} sdlog_rc_crsf_t;
...
// UART1 tuning command record: [u32 seq][ASCII bytes...][0]
typedef struct __attribute__((packed))
{
    uint32_t seq;
