...
typedef enum
{
  CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW,   //chassis will follow yaw gimbal motor angle.底盘会跟随云台角度
...
typedef struct
{
  ...
  const gimbal_motor_t *chassis_yaw_motor;   //will use the angle of yaw gimbal motor to calculate the euler angle.底盘使用到yaw云台电机的角度来计算底盘的欧拉角.
  const gimbal_motor_t *chassis_pitch_motor; //will use the angle of pitch gimbal motor to calculate the euler angle.底盘使用到pitch云台电机的角度来计算底盘的欧拉角
...
/**
  * @brief          return chassis data point (runtime)
  * @retval         chassis data point
  */
