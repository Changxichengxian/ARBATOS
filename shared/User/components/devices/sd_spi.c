...
static osMutexId sd_spi_mutex = NULL;
static osStaticMutexDef_t sd_spi_mutex_cb;
osMutexStaticDef(sd_spi_mutex, &sd_spi_mutex_cb);
...
static void sd_spi_lock(void)
{
    ...
    {
        sd_spi_mutex = osMutexCreate(osMutex(sd_spi_mutex));
    }
    if (sd_spi_mutex != NULL)
    {
        (void)osMutexWait(sd_spi_mutex, osWaitForever);
    }
}
