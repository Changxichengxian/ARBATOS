...
typedef enum
{
    BSP_UART1_RXEVENT_UNKNOWN = 0,
    BSP_UART1_RXEVENT_IDLE,
    BSP_UART1_RXEVENT_HT,
    BSP_UART1_RXEVENT_TC,
} bsp_uart1_rx_event_e;

typedef void (*bsp_uart1_rx_event_cb_t)(uint16_t size, bsp_uart1_rx_event_e evt);
typedef void (*bsp_uart1_rx_byte_cb_t)(uint8_t b);
// Return: 1 if handled (no default restart), 0 to apply default handling.
typedef uint8_t (*bsp_uart1_error_cb_t)(void);
