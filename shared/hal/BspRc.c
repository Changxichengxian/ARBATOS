...
static BspRcSbusFrame sbus_ring[BSP_RC_SBUS_RX_RING_SIZE];
...
extern void BspRcPortInit(void);

static uint8_t BspRcSbusPushFromIsr(const uint8_t frame[BSP_RC_SBUS_FRAME_LENGTH])
{
...
static void BspRcSbusNotifyFromIsr(void)
{
...
void BspRcSbusOnFrameIsr(const uint8_t *frame, uint16_t size)
{
    ...
    if (BspRcSbusPushFromIsr(frame) != 0u)
    {
        BspRcSbusNotifyFromIsr();
    }
}
