...
static uint32_t BspDwtGetPrimask(void)
{
...
static void BspDwtRestoreIrq(uint32_t primask)
{
    if (primask == 0u)
    {
        __enable_irq();
...
static void BspDwtSampleLocked(uint64_t *cycles_out)
{
...
{
    const uint32_t primask = BspDwtGetPrimask();
