...
#include "BspBuzzer.h"
#include "tim.h"
#include "BspBuzzerCfg.h"

#include <string.h>

#ifndef BSP_BUZZER_TIM_HANDLE
#error "BSP_BUZZER_TIM_HANDLE is not defined in BspBuzzerCfg.h"
#endif
...
#error "BSP_BUZZER_DMA_ID is not defined in BspBuzzerCfg.h"
#endif
#endif
...
static uint32_t BuzzerTimClockHz(void)
{
    return BspBuzzerTimClockHz();
}

static void BuzzerPwmStop(void)
{
...
void BuzzerPcmSetCarrierMinHz(uint32_t carrier_min_hz)
{
    g_buzzer_pcm_carrier_min_hz_cfg = carrier_min_hz;
}

void BuzzerPcmSetStreamGainQ8(uint16_t gain_q8)
{
...
static uint32_t BuzzerPcmCarrierMinHz(void)
{
...
static uint16_t BuzzerPcmStreamGainQ8(void)
{
...
static uint8_t BuzzerPcmIsOcPreloadEnabled(void)
{
...
static uint16_t BuzzerPcmSilenceDuty(uint16_t arr)
{
    return (uint16_t)((arr + 1u) / 2u);
}

static uint16_t BuzzerPcmU8ToDuty(uint8_t sample_u8, uint16_t arr, uint8_t volume, uint16_t gain_q8)
{
...
static void BuzzerPcmUpdateLut(uint16_t arr, uint8_t volume, uint16_t gain_q8)
{
    for (uint32_t i = 0u; i < 256u; i++)
    {
        g_buzzer_pcm_duty_lut[i] = BuzzerPcmU8ToDuty((uint8_t)i, arr, volume, gain_q8);
    }
}

static uint32_t BuzzerPcmStreamUsed(uint32_t head, uint32_t tail)
{
    return head - tail;
}

static uint32_t BuzzerPcmStreamFree(uint32_t head, uint32_t tail)
{
    return BUZZER_PCM_STREAM_RING_SIZE - BuzzerPcmStreamUsed(head, tail);
}

static uint8_t BuzzerPcmStreamPopU8(uint8_t *out_sample)
{
    if (out_sample == NULL)
    {
        return 0u;
...
static void BuzzerPcmFill(uint16_t *dst, uint32_t count)
{
    uint32_t i;
    ...
    uint16_t silence = BuzzerPcmSilenceDuty(arr);
        ...
        for (i = 0; i < count; i++)
        {
            ...
            if (sample_rate_hz != 0u && carrier_hz != 0u)
            {
                ...
                if (phase >= carrier_hz)
                {
                    ...
                    uint8_t next = 128u;
                    if (BuzzerPcmStreamPopU8(&next) != 0u)
                    {
                        cur = next;
                    }
                    else
...
int BuzzerToneStartHz(uint32_t freq_hz, uint8_t volume)
{
    ...
    return BuzzerPcmStartU8(g_buzzer_tone_pcm, g_buzzer_tone_len, sample_rate_hz, 1u, volume);
}
...
int BuzzerToneStartLegacy(uint16_t psc, uint16_t pwm)
{
    ...
    uint32_t tim_clk = BuzzerTimClockHz();
    ...
    return BuzzerToneStartHz(freq_hz, volume);
}

uint16_t BuzzerLegacyPwmHalf(void)
{
    uint32_t arr_base = (uint32_t)BSP_BUZZER_TIM_HANDLE.Init.Period;
...
int BuzzerPcmStartU8(const uint8_t *pcm_u8, uint32_t len, uint32_t sample_rate_hz, uint8_t loop, uint8_t volume)
{
    ...
    uint32_t tim_clk = BuzzerTimClockHz();
    const uint32_t carrier_min_hz = BuzzerPcmCarrierMinHz();
    ...
    g_buzzer_pcm_saved_oc_preload = BuzzerPcmIsOcPreloadEnabled();

    g_buzzer_pcm_arr = (uint16_t)arr32;
    BuzzerPcmUpdateLut(g_buzzer_pcm_arr, volume, BUZZER_PCM_GAIN_Q8_DEFAULT);
...
#if BSP_BUZZER_PCM_USE_DMA
    BuzzerPcmFill(&g_buzzer_pcm_buf[0], BUZZER_PCM_DMA_BUF_LEN);
...
int BuzzerPcmStartStreamU8(uint32_t sample_rate_hz, uint8_t volume)
{
    ...
    g_buzzer_pcm_arr = (uint16_t)arr32;
    BuzzerPcmUpdateLut(g_buzzer_pcm_arr, volume, BuzzerPcmStreamGainQ8());
    g_buzzer_pcm_sample_rate_hz = sample_rate_hz;
    g_buzzer_pcm_carrier_hz = carrier_hz;
    g_buzzer_pcm_phase = 0u;
...
uint32_t BuzzerPcmStreamWriteU8(const uint8_t *pcm_u8, uint32_t len)
{
    ...
    uint32_t free = BuzzerPcmStreamFree(head, tail);
    if (free == 0u)
    {
        return 0u;
    }
...
uint32_t BuzzerPcmStreamGetUsed(void)
{
    const uint32_t head = g_buzzer_pcm_stream_head;
    const uint32_t tail = g_buzzer_pcm_stream_tail;
    return BuzzerPcmStreamUsed(head, tail);
}

uint32_t BuzzerPcmStreamGetFree(void)
{
    const uint32_t head = g_buzzer_pcm_stream_head;
    const uint32_t tail = g_buzzer_pcm_stream_tail;
    return BuzzerPcmStreamFree(head, tail);
}

uint8_t BuzzerPcmIsStreamMode(void)
{
...
void BuzzerPcmStop(void)
{
    ...
    g_buzzer_pcm_carrier_hz = BuzzerPcmCarrierMinHz();
    g_buzzer_pcm_phase = 0u;
    g_buzzer_pcm_stream_mode = 0u;
}

uint8_t BuzzerPcmIsRunning(void)
{
...
void HAL_TIM_PWM_PulseFinishedHalfCpltCallback(TIM_HandleTypeDef *htim)
{
    ...
    BuzzerPcmFill(&g_buzzer_pcm_buf[0], BUZZER_PCM_DMA_BUF_LEN / 2u);
#else
    (void)htim;
#endif
}
...
#if BSP_BUZZER_PCM_USE_DMA
    BuzzerPcmFill(&g_buzzer_pcm_buf[BUZZER_PCM_DMA_BUF_LEN / 2u], BUZZER_PCM_DMA_BUF_LEN / 2u);
#else
    uint16_t duty = 0u;
    BuzzerPcmFill(&duty, 1u);
    __HAL_TIM_SET_COMPARE(&BSP_BUZZER_TIM_HANDLE, BSP_BUZZER_TIM_CHANNEL, duty);
...
int BuzzerToneStartHz(uint32_t freq_hz, uint8_t volume)
{
    ...
    const uint32_t tim_clk = BuzzerTimClockHz();
