...
static BspUsart6RxChunk usart6_rx_ring[BSP_USART6_RX_RING_SIZE];
...
static uint8_t BspUsart6PushFromIsr(const uint8_t *data, uint16_t len)
{
...
static void BspUsart6NotifyFromIsr(void)
{
...
{
    BspUsart6RxAttachTask(task);
}

int BspRefereeRxPop(uint8_t *out, uint16_t *out_len)
{
    return BspUsart6RxPop(out, out_len);
}

uint32_t BspRefereeRxGetDropCount(void)
{
    return BspUsart6RxGetDropCount();
}
...
void USART6_IRQHandler(void)
{
    if (USART6->SR & UART_FLAG_IDLE)
    {
        ...
        if ((huart6.hdmarx->Instance->CR & DMA_SxCR_CT) == RESET)
        {
            ...
            pushed = BspUsart6PushFromIsr(usart6_rx_dma_buf[0], this_time_rx_len);
        }
        else
        {
            __HAL_DMA_DISABLE(huart6.hdmarx);
            ...
            pushed = BspUsart6PushFromIsr(usart6_rx_dma_buf[1], this_time_rx_len);
        }

        if (pushed)
        {
            BspUsart6NotifyFromIsr();
        }
    }
}
