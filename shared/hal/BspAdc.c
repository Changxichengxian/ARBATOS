...
{
    return (fp32)BspAdcGetRaw(index) * BSP_ADC_MC02_VREF_V / BSP_ADC_MC02_FULL_SCALE;
}

uint32_t BspAdcGetStartOkCount(void)
{
    return adc1_start_ok_count;
}

uint32_t BspAdcGetStartFailCount(void)
{
...
{
    return BspAdcGetChannelVoltage(BSP_ADC_MC02_BAT_INDEX) * BSP_ADC_MC02_BAT_DIVIDER;
}
...
fp32 BspAdcGetChannelVoltage(uint8_t index)
{
    if (index == 0u)
    {
        return (fp32)BspAdcGetRaw(index) * voltage_vrefint_proportion;
    }
    return 0.0f;
}
