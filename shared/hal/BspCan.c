...
static HAL_StatusTypeDef BspCanTxWithRetry(FDCAN_HandleTypeDef *hfdcan, FDCAN_TxHeaderTypeDef *header, const uint8_t data[8])
{
...
static HAL_StatusTypeDef BspCanTxWithRetry(CAN_HandleTypeDef *hcan, CAN_TxHeaderTypeDef *header, const uint8_t data[8])
{
    uint32_t mailbox = 0u;
...
int BspCanTxFlags(uint8_t bus, uint16_t std_id, const uint8_t data[8], uint8_t dlc, uint8_t flags)
{
    ...
    ret = BspCanTxWithRetry(hfdcan, &hdr, data);
    if (last_error != NULL)
    {
        *last_error = HAL_FDCAN_GetError(hfdcan);
    }
    ...
    else
    {
        ...
        ret = BspCanTxWithRetry(hcan, &hdr, data);
        if (last_error != NULL)
        {
            *last_error = HAL_CAN_GetError(hcan);
        }
