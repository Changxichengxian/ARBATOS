...
/* 与历史 HAL 后端共用记录格式；备份 SRAM 前 512 字节专供故障证据。
 * 保证供电保持下的系统复位留存；不承诺无 VBAT 时掉电留存。 */
...
static int BspResetEvidenceEarlyCapture(void)
{
    BspResetEvidenceCaptureBoot();
    return 0;
}

SYS_INIT(BspResetEvidenceEarlyCapture, PRE_KERNEL_1, 0);
