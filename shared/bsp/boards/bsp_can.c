...
static void bsp_can_rx_push_common(uint8_t bus, uint16_t std_id, uint8_t dlc, const uint8_t data[8])
{
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t rx_fifo0_its)
{
    ...
    bsp_can_rx_push_common(bus, (uint16_t)rx_header.Identifier, bsp_can_fdcan_decode_dlc(rx_header.DataLength), rx_data);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    bsp_can_rx_push_common(bus, (uint16_t)rx_header.StdId, (uint8_t)rx_header.DLC, rx_data);
    bsp_can_rx_notify_from_isr();
}
#endif
...
{
    return (bus == 1u) ? can1_rx_drop : can2_rx_drop;
}
...
#if defined(HAL_FDCAN_MODULE_ENABLED)
    FDCAN_HandleTypeDef *hfdcan = (bus == 1u) ? &hfdcan1 : &hfdcan2;
...
{
    return (bus == 1u) ? can1_last_error : can2_last_error;
}

uint8_t bsp_can_get_last_tx_status(uint8_t bus)
{
    return (bus == 1u) ? can1_last_tx_status : can2_last_tx_status;
}

uint32_t bsp_can_get_tx_count(uint8_t bus)
{
    return (bus == 1u) ? can1_tx_count : can2_tx_count;
}

uint32_t bsp_can_get_tx_fail_count(uint8_t bus)
{
    return (bus == 1u) ? can1_tx_fail : can2_tx_fail;
}
